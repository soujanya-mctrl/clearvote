  ...
  return (
    <div className="w-full max-w-2xl space-y-12 animate-in fade-in duration-1000 pb-20">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-extrabold tracking-tighter text-white text-glow">Candidate Research</h2>
        <p className="text-zinc-600 text-[10px] font-extrabold uppercase tracking-[0.4em]">Manifesto Analysis Engine</p>
      </div>

      <div className={`w-full transition-all duration-700 ${profile || loading ? 'sticky bottom-10 z-40 px-6' : 'relative mt-4'}`}>
        <form onSubmit={(e) => handleSearch(e)} className="relative group max-w-xl mx-auto w-full">
          ...
          <input
            ...
            className="w-full px-8 py-5 bg-transparent rounded-2xl text-sm transition-all focus:border-white/20 focus:ring-0 placeholder:text-zinc-600 relative z-10"
          />
          <button
            type="submit"
        ...
        {!profile && !loading && (
          <div className="flex flex-wrap justify-center gap-3 mt-8 animate-in fade-in slide-in-from-top-4 duration-1000 delay-300">
      ...
      {profile && (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <div className="glass-card premium-gradient p-10 flex items-center gap-8">
             <div className="w-20 h-20 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center text-3xl font-extrabold text-white shadow-2xl">
                {profile.party?.[0] || profile.name?.[0]}
             </div>
             <div className="space-y-1">
                <h3 className="text-2xl font-extrabold text-white tracking-tighter text-glow">{profile.name}</h3>
                <div className="flex items-center gap-3">
                  <p className="text-sm text-zinc-500 font-medium">{profile.party}</p>
                  <div className="w-1.5 h-1.5 bg-white/10 rounded-full" />
                  <p className="text-[10px] text-zinc-600 font-extrabold uppercase tracking-widest">{profile.constituency || 'National'}</p>
                </div>
             </div>
          </div>

          <div className="space-y-6">
            <p className="text-[10px] text-zinc-700 font-extrabold uppercase tracking-[0.4em] px-4">Key Strategic Promises</p>
